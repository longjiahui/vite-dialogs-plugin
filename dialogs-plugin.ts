...
export function dialogsPlugin(options: DialogsPluginOptions): Plugin {
  ...
  function generateModuleCode(files: string[]): string {
    const dialogVueImport = toAliasImport(resolve(dir, "../Dialog.vue"));
    const lines: string[] = [
      `import { createApp } from "vue";`,
      `import { dialogControllerKey } from ${JSON.stringify(dialogVueImport)};`,
    ];
...
function mountDialog(component, props) {
  ...
  function generateDts(files: string[]): string {
    ...
    // Relative path from the .d.ts file to Dialog.vue
    let dialogVueRel = relative(dtsDir, resolve(dir, "../Dialog.vue")).replace(
      /\\/g,
      "/",
    );
    if (!dialogVueRel.startsWith(".")) dialogVueRel = `./${dialogVueRel}`;
    const dv = JSON.stringify(dialogVueRel);
    ...
    const lines: string[] = [
      ...
      `\ttype _DE<T> = import(${dv}).DialogExposed<T>;`,
      `\ttype _DS<T> = import(${dv}).DialogSettledResult<T>;`,
  ...
  return {
    ...
    load(id) {
      ...
      return generateModuleCode(scanFiles());
    },
